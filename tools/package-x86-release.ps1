# Package only freshly built additive binaries; third-party files are sidecars, never embedded.
param([switch]$Private,[string]$OutputPath='')
$ErrorActionPreference='Stop'
$root=Split-Path $PSScriptRoot -Parent
$release=Join-Path $root 'release'
& (Join-Path $PSScriptRoot 'check-additive-x86.ps1')
...
try {
    foreach($name in @('dlss5-installer-x86.exe','payload.sha256','README_PUBLIC.txt','files/dlss5-neural.addon32','files/dlss5-neural-host64.exe')){Copy-Item -LiteralPath (Join-Path $release $name) -Destination (Join-Path $stage $name)}
    if($Private){

...
un-with-timing.cmd "<game exe>"` sets it for that launch only; Steam launch options do not pass environment variables, and `setx` would leave the probe on for every process started afterwards. The frontend then logs one averaged line per 120 completed frames splitting the bridge into `input+prepare`, `host` and `output`, and names the staging path it measured. The startup line reports `probe=on`/`probe=off` so a log says which it was.

...
void paint(HWND window,HDC dc){
    ...
    if(selectedPreset!=0){
        const bool d3d8=selectedPreset==2;
        heading(dc,3,d3d8?L"D3D8 Compatibility":L"Important for D3D9",514);
        ...
        text(dc,d3d8?L"The pinned d3d8to9 layer translates D3D8 to the native D3D9 frontend.":L"during ReShade installation. No translation wrapper is required.",362,640,690,42,bodyFont,muted,DT_LEFT|DT_TOP|DT_WORDBREAK|DT_NOPREFIX);
    }
...
void drawButton(const DRAWITEMSTRUCT& d){
    ...
    if(card){
        ...
        const wchar_t* labels[]={L"D3D11 x86",L"D3D9 x86",L"D3D8 experimental"};
...
LRESULT CALLBACK proc(HWND window,UINT msg,WPARAM w,LPARAM l){
    if(msg==WM_CREATE){
        ...
        const wchar_t* names[]={L"D3D11 x86 (selected)",L"D3D9 x86",L"D3D8 experimental"};
        ...
        if(id==12||id==13){
            ...
            try {auto text=value(targetBox);install86::require(!text.empty(),"Select the target executable first");std::filesystem::path target=text;
                if(id==12){install86::require(install86::machine(install86::read(target))==0x14c,"x64 target refused: select an x86 executable");
                    ...
                    const char* presets[]={"D3D11","D3D9","D3D8"};app.install(target,presets[i]);}
